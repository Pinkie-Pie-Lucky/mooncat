...
function renderResult(){ const totals={home:0,explore:0,heart:0,tail:0}; answers.forEach(({axis,value})=>totals[axis]+=value); const district=totals.explore>=7?1:totals.heart<=4?4:totals.tail>=7?3:totals.home<=4?0:2; const record=district*4+(totals.home+totals.explore*2+totals.heart*3+totals.tail)%4+1; const result=results[record-1]; const catImage=Object.entries(catAssets).find(([path])=>path.includes(`No${String(record).padStart(2,'0')}${result[0]}.png`))?.[1]; const scenes=['它在月桂树下拍松一朵云，听见你脚步里想休息的叹息。','它在星轨站擦亮星灯，照见你望向远方又有点犹豫的眼睛。','它在雨棚下收集不同声音的雨滴，认出你细微而真诚的感受。','它在蜂蜜广场留好一只茶杯，发现你总能让人安心靠近。','它提着灯走过长长夜路，认出你认真守护的承诺。']; root.innerHTML=`<main class="app result result-scene"><section class="result-letter">${catImage?`<img class="result-cat-illustration" src="${catImage}" alt="${result[0]}">`:''}<div class="letter-top"><span>MOONLIGHT TOWN</span><span>收留来信 · No. ${String(record).padStart(2,'0')}</span></div><div class="postmark">月<br>光</div><div class="result-cat">${cat('coral')}</div><p class="letter-kicker">今夜，月亮替你找到了</p><h1>${result[0]}</h1><p class="district">来自 ${result[1]}</p><div class="gold-rule">✦</div><div class="letter-lines"><p><b>你们相遇的地方：</b>${scenes[district]}</p><p><b>今夜的你：</b>${result[2]}</p><p><b>它会怎样陪你：</b>它不催你说话，只把尾巴轻轻搭在你的手背上，陪你把这一段月光慢慢走完。</p><p><b>它的专属小魔法：</b>${result[3]}</p></div><blockquote>“今晚不用赶路。<br>我会陪你把月光走完。”</blockquote><div class="certificate"><b>月亮小镇收留凭证</b><span>称号：${result[1]}的月光旅人</span><span>陪伴有效期：每一个想回家的夜晚</span></div></section></main>`; }
renderIntro();
